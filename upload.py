...
def uploadToDir(sFile, sDir, token=None, sToken=None, mMeta=None, isUnique=False):
    global TOKENFILE;
    if (not token):
        if (not sToken):
            assert TOKENFILE;
    ...
    assert token;
    mMeta = mMeta or {};
    dir1 = getDir(sDir, token);
    if (isUnique):
        sName = split(sFile)[1];
        aFiles = dir1.getChildren(sName);
        if (aFiles):
    ...
    file = File(sFile, token, mMeta=mMeta);
    isDone = False;
    while not isDone:
        try:
...
def upload(sFile):
    ...
    sFileId = uploadToDir(sFile, sDir, token, isUnique=isUnique);
    if (sFileId):
